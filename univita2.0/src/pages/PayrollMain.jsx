...
import {
  ...
  X, CheckCircle, Download, Eye, DollarSign, Users, TrendingUp
} from 'lucide-react';
...
const PayrollMain = ({ setView }) => {
  ...
  const computePayroll = (emp) => {
    const att = attendanceSummary[emp.employee_id] || { regularHours: 0, overtimeHours: 0, leaveDays: 0 };
    ...
    const extras = employeeExtras[emp.employee_id] || { transport: 0, meal: 0, housing: 0, loans: 0, other: 0 };

    const allowances = (extras.transport || 0) + (extras.meal || 0) + (extras.housing || 0);
    const grossPay = monthlySalary + overtimePay + allowances;
  ...
  const updateExtras = (empId, field, value) => {
    setEmployeeExtras(prev => ({
      ...prev,
      [empId]: { ...prev[empId], [field]: value === '' ? 0 : parseFloat(value) || 0 }
    }));
  };
  ...
  const printPayslip = (emp) => {
    ...
    printWindow.document.write(`
      ...
      <table class="payslip-table">
        ...
        <tr><th>Position</th><td>${emp.role || 'Instructor'}</td></tr>
        ...
        <tr><th>SSS</th><td>₱${calc.sss.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td></tr>
        <tr><th>PhilHealth</th><td>₱${calc.philHealth.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td></tr>
        <tr><th>Pag-IBIG</th><td>₱${calc.pagIbig.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td></tr>
  ...
  const exportToCSV = () => {
    const headers = [
      "Full Name","Employee ID","Regular Hours","Overtime Hours","Monthly Salary",
      "Overtime Pay","Transport Allowance","Meal Allowance","Housing Allowance",
    ...
    filteredEmployees.forEach(emp => {
      ...
      const extras = employeeExtras[emp.employee_id] || {};
      rows.push([
        emp.full_name, emp.employee_id, calc.regularHours, calc.overtimeHours,
  ...
  return (
    <div className="card payroll-main-card">
        ...
        <>
          <div className="pm-summary-row">
            ...
            <div className="summary-card">
              ...
              <div>
                ...
                <div className="summary-value">₱{summary.totalGross.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
              </div>
            </div>
            <div className="summary-card">
              ...
              <div>
                ...
                <div className="summary-value">₱{summary.totalTax.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
              </div>
            </div>
            <div className="summary-card">
              ...
              <div>
                ...
                <div className="summary-value">₱{summary.totalNet.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
              </div>
            </div>
          </div>
          ...
          <div className="pm-actions-row">
            ...
            <div className="flex-gap-2">
              <button className="btn-monthly-payroll" onClick={() => setShowMonthlyModal(true)}>
                <CalendarDays size={18} /> Process Month
              </button>
              <button className="btn-view-history" onClick={() => setView('payroll-history')}>
                    ...
                    filteredEmployees.map(emp => {
                      ...
                      return (
                        <tr key={emp.employee_id}>
                          ...
                          <td>{att.regularHours} hrs</td>
                          <td className="text-right">
                            <div className="action-buttons-inline">
                              <button className="btn-payslip" onClick={() => openPayslip(emp)}>
                                <Eye size={14} />
                              </button>
      ...
      <FormalModal
        ...
        footer={
          <>
            <button className="btn-modal-cancel" onClick={() => setShowMonthlyModal(false)}>Cancel</button>
            <button className="btn-modal-submit" onClick={handleMonthlyPayroll} disabled={isProcessing}>
      ...
      >
        ...
        <div className="modal-form-group">
          ...
          <select value={selectedMonth} onChange={(e) => setSelectedMonth(parseInt(e.target.value))} className="modal-select">
        ...
        <div className="modal-form-group">
          ...
          <select value={selectedYear} onChange={(e) => setSelectedYear(parseInt(e.target.value))} className="modal-select">
      ...
      <FormalModal
        ...
        title={`Payslip – ${payslipEmployee?.full_name || ''}`}
        wide
        footer={
          <>
            <button className="btn-modal-cancel" onClick={() => { setShowPayslipModal(false); setPayslipEmployee(null); }}>Close</button>
            <button className="btn-print" onClick={() => printPayslip(payslipEmployee)}>Print</button>
          </>
        }
      >
        {payslipEmployee && (() => {
          ...
          return (
            <div className="payslip-body">
              ...
              <div className="payslip-row">
                <span>Transport</span>
                {eligible ? (
                  <input type="number" min="0" value={extras.transport || ''} onChange={e => updateExtras(emp.employee_id, 'transport', e.target.value)} className="payslip-edit-input" />
                ) : <span>₱0.00</span>}
              </div>
              <div className="payslip-row">
                <span>Meal</span>
                {eligible ? (
                  <input type="number" min="0" value={extras.meal || ''} onChange={e => updateExtras(emp.employee_id, 'meal', e.target.value)} className="payslip-edit-input" />
                ) : <span>₱0.00</span>}
              </div>
              <div className="payslip-row">
                <span>Housing</span>
                {eligible ? (
                  <input type="number" min="0" value={extras.housing || ''} onChange={e => updateExtras(emp.employee_id, 'housing', e.target.value)} className="payslip-edit-input" />
                ) : <span>₱0.00</span>}
              </div>
              <hr/>
              ...
              <div className="payslip-row">
                <span>SSS</span>
                <input type="number" min="0" value={extras.sssOverride != null ? extras.sssOverride : calc.sss} onChange={e => updateExtras(emp.employee_id, 'sssOverride', e.target.value)} className="payslip-edit-input" />
              </div>
              <div className="payslip-row">
                <span>PhilHealth</span>
                <input type="number" min="0" value={extras.philHealthOverride != null ? extras.philHealthOverride : calc.philHealth} onChange={e => updateExtras(emp.employee_id, 'philHealthOverride', e.target.value)} className="payslip-edit-input" />
              </div>
              <div className="payslip-row">
                <span>Pag-IBIG</span>
                <input type="number" min="0" value={extras.pagIbigOverride != null ? extras.pagIbigOverride : calc.pagIbig} onChange={e => updateExtras(emp.employee_id, 'pagIbigOverride', e.target.value)} className="payslip-edit-input" />
              </div>
              <div className="payslip-row">
                <span>Loans</span>
                <input type="number" min="0" value={extras.loans || ''} onChange={e => updateExtras(emp.employee_id, 'loans', e.target.value)} className="payslip-edit-input" />
              </div>
              <div className="payslip-row">
                <span>Other</span>
                <input type="number" min="0" value={extras.other || ''} onChange={e => updateExtras(emp.employee_id, 'other', e.target.value)} className="payslip-edit-input" />
              </div>
